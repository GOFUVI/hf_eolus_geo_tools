...
Usage:
  python merge_parquet.py --root /data

Behavior:
- For every directory under --root that contains one or more Parquet files,
  merge/rename them into a single Parquet file:
  - If the directory name looks like "key=value", output file is "value.parquet".
  - Otherwise (no partitions), output file is "data.parquet" in that directory.
- Non-Parquet, hidden, and marker files (e.g. _SUCCESS) are ignored.
"""
